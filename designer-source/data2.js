// Additional mock data for Conversas, Clientes, Cardápio, etc.
...
const CONVERSAS = [
  ...
  { id: 'c-4', customer: 'Rafael Almeida', phone: '(45) 99834-1188', initial: 'RA', avatarColor: '#16a34a',
    lastMsg: 'Já recebi, valeu! 🙏', timeAgo: 'há 23min', unread: 0, status: 'ai',
    msgs: [],
  },
  { id: 'c-5', customer: 'Letícia Marinho', phone: '(45) 99509-6680', initial: 'LM', avatarColor: '#f59e0b',
    lastMsg: 'O entregador já saiu?', timeAgo: 'há 31min', unread: 1, status: 'ai', pedidoOpen: true,
    msgs: [],
  },
  { id: 'c-6', customer: 'João Pedro Vasques', phone: '(45) 99427-3019', initial: 'JV', avatarColor: '#8b5cf6',
    lastMsg: 'Tudo certo, obrigado!', timeAgo: 'há 38min', unread: 0, status: 'ai',
    msgs: [],
  },
  { id: 'c-7', customer: 'Patricia Lemos', phone: '(45) 99214-8800', initial: 'PL', avatarColor: '#ec4899',
    lastMsg: 'qual o sabor do milk shake?', timeAgo: 'há 1min', unread: 3, status: 'ai',
    msgs: [],
  },
  { id: 'c-8', customer: 'Henrique Cardoso', phone: '(45) 99334-7720', initial: 'HC', avatarColor: '#0891b2',
    lastMsg: 'O sistema de cupons funciona ainda?', timeAgo: 'há 4min', unread: 1, status: 'escalated',
    msgs: [],
  },
];

const ESCALACOES = [
  { id: 'e-1', customer: 'Diego Ferreira', phone: '(45) 99715-2204', motivo: 'Troco · valor incomum', preview: 'Posso pagar em dinheiro? Vou precisar de troco', aberta: '12min', atendente: null, priority: 'normal' },
  { id: 'e-2', customer: 'Henrique Cardoso', phone: '(45) 99334-7720', motivo: 'Cupom não reconhecido', preview: 'O sistema de cupons funciona ainda?', aberta: '4min', atendente: null, priority: 'normal' },
  { id: 'e-3', customer: 'Beatriz Schäfer', phone: '(45) 99127-3344', motivo: 'Pedido extraviado', preview: 'Meu pedido nunca chegou ontem...', aberta: '38min', atendente: 'Cristiano', priority: 'high' },
];
...
const RELATORIOS_HEATMAP = (() => {
  // 7 dias × 24h
  const data = [];
  for (let day = 0; day < 7; day++) {
    for (let h = 0; h < 24; h++) {
      let v = 0;
      if (h >= 11 && h <= 14) v = Math.floor(2 + Math.random() * 4);       // lunch
      else if (h >= 18 && h <= 22) v = Math.floor(5 + Math.random() * 10);  // dinner peak
      else if (h >= 15 && h <= 17) v = Math.floor(1 + Math.random() * 3);
      else if (h >= 9 && h <= 10) v = Math.floor(0 + Math.random() * 2);
      if (day >= 4 && h >= 20 && h <= 22) v += Math.floor(3 + Math.random() * 5); // weekend boost
      data.push({ day, h, v });
    }
  }
  return data;
...
window.MOCK2 = { CONVERSAS, ESCALACOES, CLIENTES, CARDAPIO, ZONAS, AI_AUDIT, AI_GUARDRAILS, RELATORIOS_BY_DAY, RELATORIOS_BAIRROS, RELATORIOS_CATEGORIAS, RELATORIOS_HEATMAP };

...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await sb.from('sabores').update(updates).eq('id', params.id).eq('tenant_id', ctx.tenant_id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  void logAdminAction(ctx, 'update_sabor', 'sabores', params.id, updates)
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const ctx = await requireAdmin(req); if (!isAdminCtx(ctx)) return ctx
  const sb = supabaseService()
  const { error } = await sb.from('sabores').delete().eq('id', params.id).eq('tenant_id', ctx.tenant_id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  void logAdminAction(ctx, 'delete_sabor', 'sabores', params.id, null)

...
import { sendFacebook, verifyMetaWebhook } from '@/lib/meta'
    ...
    for (const messaging of (entry.messaging || [])) {
      ...
      try {
        ...
        await sb.from('conversas').insert({
          ...
          provider_message_id: messaging.message?.mid, channel: 'facebook',
        })
        const result = await orchestrate({
          telefone: senderId, message: text, channel: 'facebook',
        ...
        await sb.from('conversas').insert({
          ...
          agent_used: result.specialist, channel: 'facebook',
        ...
        try { await sendFacebook(pageId, senderId, result.reply) }
        catch (e: any) { console.warn('[fb] send err', e?.message) }
      } catch (e: any) { console.error('[fb] err', e?.message) }
    }
  }
